...
- For event creation, prefer `admin-create-event` over `admin-create-record` to avoid raw UUID-heavy payloads and get event-specific confirmation text. Its payload is aligned with `/hantar-majlis` field semantics (excluding media-only UX concerns): use route-key helpers (`organizer_key`, `institution_key`, `venue_key`, `space_key`, `speaker_keys`, `reference_keys`) plus direct arrays for tags (`domain_tags`, `discipline_tags`, `source_tags`, `issue_tags`), `other_key_people`, and `series` where needed.
