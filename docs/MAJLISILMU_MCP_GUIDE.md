...
3. Upload the result with `admin-upload-event-cover-image` or `admin-upload-event-poster-image` (admin) / `member-upload-event-cover-image` or `member-upload-event-poster-image` (member) by passing the `event_key` and an explicit image descriptor (`{content_base64, filename}` recommended in proxied clients, or `{download_url, file_id, filename}` when URL fetch is available).
