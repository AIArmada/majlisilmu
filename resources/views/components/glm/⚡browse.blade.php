        ...
        <div class="grid lg:grid-cols-2 gap-12">
            ...
            <div>
                <div class="flex items-center gap-3 mb-8">
                    ...
                    <div>
                        <h2 class="text-2xl font-bold text-slate-900">{{ __('Cari Mengikut Negeri') }}</h2>
                        <p class="text-slate-500 text-sm">{{ __('Majlis ilmu berdekatan dengan anda') }}</p>
                    </div>
                </div>

                @if($this->popularStates->isNotEmpty())
                    <div class="grid grid-cols-2 gap-3">
                        @foreach($this->popularStates as $state)
                            <a href="{{ route('events.index', ['state_id' => $state->id]) }}" wire:navigate
                                ...
                                <span class="text-sm font-bold text-slate-400 group-hover:text-blue-600 bg-white group-hover:bg-blue-100 px-2 py-0.5 rounded-full transition-colors">
                                    {{ $state->events_count }}
                                </span>
                            </a>
                        @endforeach
                ...
                @else
                    <div class="text-center py-8 rounded-xl bg-slate-50 text-slate-500">
                        {{ __('Tiada majlis di negeri buat masa ini') }}
                    </div>
                @endif

                <a href="{{ route('events.index') }}" wire:navigate
                    class="inline-flex items-center gap-2 mt-6 text-blue-600 font-semibold hover:text-blue-700 transition-colors">
            ...
            <div>
                <div class="flex items-center gap-3 mb-8">
                    ...
                    <div>
                        <h2 class="text-2xl font-bold text-slate-900">{{ __('Cari Mengikut Tajuk') }}</h2>
                        <p class="text-slate-500 text-sm">{{ __('Majlis popular berdasarkan tajuk') }}</p>
                    </div>
                </div>

                @if($this->popularTitles->isNotEmpty())
                        ...
                        @foreach($this->popularTitles as $item)
                            <a href="{{ route('events.index', ['search' => $item->title]) }}" wire:navigate
                                ...
                                @if($item->events_count > 1)
                                    <span class="text-xs bg-purple-200 text-purple-800 px-2 py-0.5 rounded-full">{{ $item->events_count }}</span>
                                @endif
                            </a>
                        @endforeach
                    </div>
                @else
                    <div class="text-center py-8 rounded-xl bg-slate-50 text-slate-500">
                        {{ __('Tiada tajuk popular buat masa ini') }}
                    </div>
                @endif

                <a href="{{ route('events.index') }}" wire:navigate
                    class="inline-flex items-center gap-2 mt-6 text-purple-600 font-semibold hover:text-purple-700 transition-colors">
