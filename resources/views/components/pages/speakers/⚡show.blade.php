        ...
        <div class="container relative z-10 mx-auto mt-0 max-w-6xl px-4 pb-16 pt-8 sm:px-6 lg:px-8">
            ...
            @if($hasSocialLinks || $hasPublicContacts || $showPendingStatusNotice || $showJoinCta || $hasInspiration || $speaker->is_freelance)
                ...
                <div class="scroll-reveal reveal-right rounded-2xl border border-slate-200/70 bg-slate-50/80 p-4 shadow-sm" x-intersect.once="$el.classList.add('revealed')">
                    ...
                    @php($speakerClaimRouteSegment = \App\Enums\MemberSubjectType::Speaker->publicRouteSegment())
                    @php($viewer = auth()->user())
                    @php($isSpeakerMember = $viewer instanceof \App\Models\User ? $speaker->members()->whereKey($viewer->getKey())->exists() : false)
                    @php($hasPendingSpeakerClaim = $viewer instanceof \App\Models\User && !$isSpeakerMember
                        ? $viewer->membershipClaims()
                            ->where('subject_type', \App\Enums\MemberSubjectType::Speaker)
                            ->where('subject_id', $speaker->getKey())
                    ...
                    <div class="mt-3 flex flex-wrap gap-2">
                        @if(!$isSpeakerMember && !$hasPendingSpeakerClaim)
                            <a href="{{ route('membership-claims.create', ['subjectType' => $speakerClaimRouteSegment, 'subjectId' => $speaker->slug]) }}" wire:navigate
