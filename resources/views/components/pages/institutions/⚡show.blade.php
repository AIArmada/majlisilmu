                ...
                <div class="space-y-6 p-6 sm:p-8">
                    <article class="overflow-hidden rounded-2xl border border-slate-200/60 bg-white shadow-lg shadow-slate-200/40">
                        <div class="relative h-56 overflow-hidden bg-slate-100">
                            <img src="{{ $sharePreviewImage }}" alt="{{ $institution->name }}" class="size-full {{ $sharePreviewHasCover ? 'object-cover' : 'object-contain' }}" loading="lazy">
                            <div class="absolute inset-0 bg-gradient-to-t from-slate-900/55 to-transparent"></div>
                        </div>
                        <div class="p-5">
                            <h4 class="font-heading text-lg font-bold leading-tight text-slate-900">{{ $institution->name }}</h4>
                            @if($locationString)
