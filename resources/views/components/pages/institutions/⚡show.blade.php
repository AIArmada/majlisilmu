        ...
        <div class="flex flex-col gap-8 lg:grid lg:grid-cols-[1fr_340px]">
            ...
            <div class="institution-sidebar-column order-1 space-y-6 lg:order-2 lg:sticky lg:top-6 lg:self-start">
                ...
                <div class="scroll-reveal reveal-right rounded-2xl border border-slate-200/70 bg-slate-50/80 p-4 shadow-sm"
                    ...
                    @php($institutionClaimRouteSegment = \App\Enums\MemberSubjectType::Institution->publicRouteSegment())
                    @php($viewer = auth()->user())
                    @php($isInstitutionMember = $viewer instanceof \App\Models\User ? $institution->members()->whereKey($viewer->getKey())->exists() : false)
                    @php($hasPendingInstitutionClaim = $viewer instanceof \App\Models\User && !$isInstitutionMember
                        ? $viewer->membershipClaims()
                            ->where('subject_type', \App\Enums\MemberSubjectType::Institution)
                            ->where('subject_id', $institution->getKey())
                            ->where('status', \App\Enums\MembershipClaimStatus::Pending)
                            ->exists()
                        : false)
                    ...
                    <div class="mt-3 flex flex-wrap gap-2">
                        @if(!$isInstitutionMember && !$hasPendingInstitutionClaim)
                            <a href="{{ route('membership-claims.create', ['subjectType' => $institutionClaimRouteSegment, 'subjectId' => $institution->slug]) }}" wire:navigate
                                class="inline-flex items-center gap-1.5 rounded-lg border border-emerald-200 bg-white px-3 py-2 text-xs font-semibold text-emerald-700 transition hover:border-emerald-300 hover:bg-emerald-50 hover:text-emerald-800">
                                {{ __('Tuntut Keahlian') }}
                            </a>
                        @elseif($hasPendingInstitutionClaim)
                            <span class="inline-flex items-center gap-1.5 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-700">
                                {{ __('Tuntutan Keahlian Dihantar') }}
                            </span>
                        @endif
