                                    ...
                                    <article class="flex flex-col md:flex-row gap-6 bg-white rounded-3xl p-6 border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-emerald-500/10 hover:-translate-y-1 transition-all group">
                                         <div class="md:w-48 h-32 md:h-auto rounded-2xl bg-slate-100 relative overflow-hidden flex-shrink-0">
                                            @if($event->card_image_url)
                                                <img src="{{ $event->card_image_url }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                            @else
                                            ...
                                            <div class="absolute top-2 left-2 inline-flex flex-col items-center justify-center bg-white/90 backdrop-blur-sm rounded-lg px-2 py-1 shadow-sm border border-white/50 min-w-[3rem]">
                                                <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">{{ \App\Support\Timezone\UserDateTimeFormatter::translatedFormat($event->starts_at, 'M') }}</span>
                                                <span class="text-lg font-black text-slate-900 leading-none">{{ \App\Support\Timezone\UserDateTimeFormatter::format($event->starts_at, 'd') }}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex-grow flex flex-col justify-center">
                                            ...
                                            <div class="flex items-center gap-4 text-sm text-slate-500 mb-4">
                                                <span class="flex items-center gap-1.5">
                                                    ...
                                                    {{ \App\Support\Timezone\UserDateTimeFormatter::translatedFormat($event->starts_at, 'l, h:i A') }}
                                                </span>
                                            </div>
