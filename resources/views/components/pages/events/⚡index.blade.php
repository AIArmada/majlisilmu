    ...
    <div class="relative min-h-screen pb-32">
        ...
        <div class="relative pt-24 pb-16 bg-white border-b border-slate-100 overflow-hidden">
            ...
            <div class="container relative mx-auto px-6 lg:px-12 text-center">
                <span class="inline-flex items-center gap-1.5 rounded-full border border-emerald-100 bg-white/80 px-3 py-1 text-xs font-semibold text-emerald-700 shadow-sm backdrop-blur-sm mb-6">
                ...
                <h1 class="font-heading text-5xl md:text-6xl font-extrabold text-slate-900 tracking-tight text-balance mb-6">
                    ...
                    <span class="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-teal-500">Circle of Knowledge</span>
                </h1>
                <p class="text-slate-500 text-lg md:text-xl max-w-2xl mx-auto text-balance">
                    Discover classes, lectures, and community gatherings happening near you. Connect with knowledge seekers in your area.
                </p>
            </div>
        </div>
        ...
        <div class="container mx-auto px-6 lg:px-12 -mt-8 relative z-10">
            ...
            <form action="{{ route('events.index') }}" method="GET" x-ref="form" x-data="{
                locating: false,
                showFilters: {{ request()->hasAny(['state_id', 'language', 'genre', 'audience']) ? 'true' : 'false' }},
                locate() {
                    ...
                    <div class="flex gap-2">
                        <button type="button" @click="locate" :disabled="locating"
                            class="h-14 px-6 rounded-2xl border-2 border-slate-100 bg-white font-semibold text-slate-600 hover:border-emerald-500 hover:text-emerald-600 focus:ring-4 focus:ring-emerald-500/10 transition-all flex items-center justify-center gap-2 min-w-[140px]">
                             <svg class="w-5 h-5" :class="locating ? 'animate-spin' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path x-show="!locating" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                                <path x-show="!locating" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                                <circle x-show="locating" class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path x-show="locating" class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                            </svg>
                 ...
                 <div class="flex items-center justify-between border-t border-slate-100 pt-5">
                    <button type="button" @click="showFilters = !showFilters"
                        ...
                        <span class="flex items-center justify-center w-8 h-8 rounded-lg bg-emerald-50">
                            <svg class="w-5 h-5 transition-transform duration-300" :class="{ 'rotate-180': showFilters }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div x-show="showFilters" x-collapse>
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 pt-6 mt-2">
                        <!-- State Filter -->
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-slate-400 uppercase tracking-wider">{{ __('Location') }}</label>
                        ...
                        <!-- Language Filter -->
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-slate-400 uppercase tracking-wider">{{ __('Language') }}</label>
                        ...
                        <!-- Genre Filter -->
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-slate-400 uppercase tracking-wider">{{ __('Event Type') }}</label>
                            <select name="genre" onchange="this.form.submit()"
                                class="w-full h-11 px-3 rounded-xl border border-slate-200 bg-slate-50 text-sm font-medium focus:border-emerald-500 focus:ring-0 cursor-pointer hover:border-emerald-400 transition-colors">
                                <option value="">{{ __('Any Type') }}</option>
                                <option value="kuliah" @selected(request('genre') == 'kuliah')>Kuliah</option>
                                <option value="ceramah" @selected(request('genre') == 'ceramah')>Ceramah</option>
                                <option value="tazkirah" @selected(request('genre') == 'tazkirah')>Tazkirah</option>
                                <option value="forum" @selected(request('genre') == 'forum')>Forum</option>
                                <option value="halaqah" @selected(request('genre') == 'halaqah')>Halaqah</option>
                            </select>
                        </div>
                        
                        <!-- Audience Filter -->
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-slate-400 uppercase tracking-wider">{{ __('Audience') }}</label>
                            <select name="audience" onchange="this.form.submit()"
                                class="w-full h-11 px-3 rounded-xl border border-slate-200 bg-slate-50 text-sm font-medium focus:border-emerald-500 focus:ring-0 cursor-pointer hover:border-emerald-400 transition-colors">
                                <option value="">{{ __('Everyone') }}</option>
                                <option value="general" @selected(request('audience') == 'general')>General</option>
                                <option value="men_only" @selected(request('audience') == 'men_only')>Men Only</option>
                                <option value="women_only" @selected(request('audience') == 'women_only')>Women Only</option>
                                <option value="youth" @selected(request('audience') == 'youth')>Youth</option>
                                <option value="children" @selected(request('audience') == 'children')>Children</option>
                                <option value="families" @selected(request('audience') == 'families')>Families</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                @if(request()->hasAny(['search', 'state_id', 'language', 'genre', 'audience', 'lat']))
                    <div class="flex flex-wrap items-center gap-2 mt-6 pt-4 border-t border-slate-100">
                        ...
                        @endif
                        <a href="{{ route('events.index') }}" wire:navigate class="ml-auto text-xs font-bold text-red-500 hover:text-red-600 hover:underline">
