                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            @if($event->institution)
                                <a href="{{ route('institutions.show', $event->institution) }}" wire:navigate
                                    ...
                                    @php
                                        $institutionChipImageUrl = $event->institution->getFirstMediaUrl('cover', 'banner') ?: $event->institution->getFirstMediaUrl('logo');
                                    @endphp
                                    @if($institutionChipImageUrl)
                                        <img src="{{ $institutionChipImageUrl }}" alt="{{ $event->institution->name }}" class="aspect-video w-6 rounded object-cover ring-1 ring-white/30" width="24" height="14" loading="lazy">
                                    @endif
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="font-bold text-slate-900">{{ \App\Support\Timezone\UserDateTimeFormatter::translatedFormat($event->starts_at, 'l, F j, Y') }}</p>
                                ...
                                @if($nextSession)
                                    <p class="mt-1 text-xs text-emerald-700">
                                        {{ __('Next session') }}:
                                        {{ \App\Support\Timezone\UserDateTimeFormatter::translatedFormat($nextSession->starts_at, 'd M Y, h:i A') }}
                                    </p>
                                @endif
                            </div>
                        </div>
                                ...
                                <div class="space-y-2">
                                    @foreach($upcomingSessions->take(5) as $session)
                                        <div class="rounded-lg border border-slate-100 bg-slate-50 px-3 py-2">
                                            ...
                                            @if($session->ends_at)
                                                <p class="text-xs text-slate-500">{{ __('Ends') }} {{ \App\Support\Timezone\UserDateTimeFormatter::format($session->ends_at, 'h:i A') }}</p>
                                            @endif
                                        </div>
                                    @endforeach
                                </div>
                    ...
                    <div class="mt-6 space-y-3">
                        @auth
                            ...
                            <button type="button" wire:click="toggleGoing" wire:loading.attr="disabled"
                                ...
                                <span class="font-bold">{{ $isGoing ? __('Hadir') : __('Akan Hadir') }}</span>
                            </button>
