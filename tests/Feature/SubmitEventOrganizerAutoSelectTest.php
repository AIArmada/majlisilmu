...
it('auto-selects the speaker as an event speaker when selected as organizer', function () {
    ...
    Livewire::test('pages.submit-event.create')
        ...
        ->set('data.speaker_organizer_id', $speaker->id)
        ->assertSet('data.speakers', [$speaker->id])
        ->set('data.title', 'Auto Select Speaker Event')
        ->set('data.start_date', now()->addDay()->toDateString())
        ->set('data.event_time', '20:00')
        ->set('data.time_option', 'other')
        ...
        ->set('data.topics', [\App\Models\Topic::factory()->create()->id])
        ->set('data.submitter_name', 'Test User')
        ->set('data.submitter_email', 'test@example.com')
        ->set('data.venue', \App\Models\Venue::factory()->create(['status' => 'verified'])->id)
        ->call('submit')
        ->assertHasNoErrors();
