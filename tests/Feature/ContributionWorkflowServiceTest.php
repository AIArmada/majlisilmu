...
it('creates staged pending institution records with structured relation data', function () {
    ...
    expect($institution->status)->toBe('pending')
        ...
        ->and($institution->members()->whereKey($proposer->id)->exists())->toBeTrue();
});
...
it('approves institution create requests and promotes the proposer to owner', function () {
    $proposer = User::factory()->create();
    ...
    expect($approvedRequest->status)->toBe(ContributionRequestStatus::Approved)
        ...
        ->and($institution->members()->whereKey($proposer->id)->exists())->toBeTrue()
        ->and(app(MemberPermissionGate::class)->canInstitution($proposer, 'institution.update', $institution))->toBeTrue();
});
