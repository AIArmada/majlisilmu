...
it('keeps the MCP guide aligned with the verified admin and member write-capable resource sets', function () {
    ...
    expect(documentedMcpGuideBulletList($markdown, 'Current structurally write-capable admin resources include:'))
        ...
        ->toContain('A client connected to `/mcp/admin` sees only the shared docs tools (`search`, `fetch`) plus `admin-*` tools. It will not see any `member-*` tools.')
        ->toContain('A client connected to `/mcp/member` sees only the shared docs tools plus `member-*` tools. It will not see any `admin-*` tools.')
        ->toContain('Missing `member-*` tools from an `/mcp/admin` connector is expected, not evidence that the member server is unregistered.')
        ...
        ->toContain('The `url` returned by `search` and shown in the raw MCP resource list is informational. The `fetch` tool schema accepts `id` only; call it as `{"id":"docs-mcp-guide"}` and do not pass `url` or `file://...`.')
        ...
        ->toContain('`admin-list-resources` is a discovery manifest, not merely a small name list. Keep `verbose=false` for compact exploration and use `verbose=true` only when you need full metadata.')
