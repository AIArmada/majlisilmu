...
#[IsDestructive(true)]
#[IsOpenWorld(true)]
class MemberGenerateEventPosterImageTool extends AbstractMemberTool
{
    use GeneratesEventImageResponse;

    protected string $name = 'member-generate-event-poster-image';

    protected string $title = 'Generate Event Poster Image';

    protected string $description = 'Generate and save a 4:5 portrait marketing poster for one accessible Ahli event. The tool builds a prompt from event data, relation data, and selected available media, attaches suitable reference images, generates the image, normalizes it to 4:5, and stores it in the Event poster media collection.';
    ...
    public function __construct(
        ...
        $this->setMeta([
            'openai/toolInvocation/invoking' => 'Generating event poster image...',
            'openai/toolInvocation/invoked' => 'Event poster image generated.',
        ]);
    }
    ...
    public function handle(Request $request): ResponseFactory|Response
    {
        return $this->safeResponse(function () use ($request): ResponseFactory {
            ...
            return $this->eventImageResponse($this->imageGenerationService->generate($event, 'poster', [
