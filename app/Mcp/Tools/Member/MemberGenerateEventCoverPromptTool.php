...
#[IsReadOnly]
#[IsIdempotent]
class MemberGenerateEventCoverPromptTool extends AbstractMemberTool
{
    use ReturnsEventCoverPromptResponse;

    protected string $name = 'member-generate-event-cover-prompt';

    protected string $title = 'Generate Event Cover Prompt';

    protected string $description = 'Use this when an Ahli member asks ChatGPT to create a new Majlis Ilmu event cover/poster image for an event they can access. It returns a ready image-generation prompt, selected reference images, and direct event/relation/media context. Do not use this to save the generated poster; after user approval, use member-update-record with the Event poster media field.';
