...
#[IsDestructive(true)]
#[IsOpenWorld(true)]
class AdminGenerateEventPosterImageTool extends AbstractAdminTool
{
    use GeneratesEventImageResponse;

    protected string $name = 'admin-generate-event-poster-image';

    protected string $title = 'Generate Event Poster Image';

    protected string $description = 'Generate and save a 4:5 portrait marketing poster for one admin-accessible event. The tool builds a prompt from event data, relation data, and selected available media, attaches suitable reference images, generates the image, normalizes it to 4:5, and stores it in the Event poster media collection.';
