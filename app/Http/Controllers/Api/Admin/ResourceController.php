...
class ResourceController extends Controller
{
    ...
    #[QueryParameter('validate_only', 'When true, validates and normalizes the payload without persisting changes. Successful responses return a preview envelope; validation failures also return machine-readable remediation details such as `fix_plan`, `remaining_blockers`, `normalized_payload_preview`, and `can_retry`.', required: false, type: 'boolean', infer: false, default: false, example: false)]
    #[Endpoint(
    ...
    #[PathParameter('resourceKey', 'Writable admin resource key from `GET /admin/manifest`.', example: 'speakers')]
    #[PathParameter('recordKey', 'Existing admin record route key returned by the collection or record endpoints.', example: '0195b86a-3c15-73fa-a2d8-5a45f6a7f701')]
    #[QueryParameter('validate_only', 'When true, validates and normalizes the payload without persisting changes. Successful responses include the current record snapshot and destructive media clear-flag warnings; validation failures also return machine-readable remediation details such as `fix_plan`, `remaining_blockers`, `normalized_payload_preview`, and `can_retry`.', required: false, type: 'boolean', infer: false, default: false, example: false)]
    #[Endpoint(
